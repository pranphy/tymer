...
#let slide(
  ...
  context {
    let loc = here() // Get the location where this slide is being laid out

    if slide-ct.get().first() > 0 { pagebreak() }
...
#let slide-number(obj) = {
  context {
    let current = if backup-st.get() { backup-no.get().at(0) } else { slide-ct.get().at(0) }
    let total = if backup-st.get() { backup-no.final().at(0) } else { slide-ct.final().at(0) }
    [#(current - 1)/#(total - 1)]
  }
}
...
#let slides-default-theme() = data => {
  let title-slide(slide-info, bodies,footer: false) = {
    ...
    align(center + horizon)[
      #block( stroke: ( y: 1mm + data.thm.pr), inset: 1em, breakable: false, [
...
let default(slide-info, bodies,content:none,footer:true) = {
  ...
  if "title" in slide-info {
    block(
      width: 100%, inset: (x: 1em, y: 0.5em), breakable: false, fill: data.thm.pr,
  ...
  let slide_footer() = {
    block(width:100%, fill: data.thm.pr.darken(90%))[
      #text(fill:data.thm.fg.darken(20%), size:12pt)[
        #h(0.08fr)
        #cbox(fill: data.thm.sec,[
          #(data.short-authors.at(0)) (#data.institute.short)
        ])
        #h(1fr)
        //#cbox(data.date)
        #cbox(width: 50%, fill: data.thm.bg.darken(20%),[#text(data.thm.fg.darken(10%),data.title) ])
        #h(1fr)
        #cbox(fill: data.thm.sec,al: right,[#data.date #h(5em)  #slide-number(slide-ct)])]
        #h(0.15fr)
      ]
    }
...
#let slides(
  ...
  aspect: 1.6,
  thm: (fg:white,bg:rgb("282a36"), pr:orange, sec:black),
  body
) = {
  ...
  let data = (
    ...
    authors: if type(authors) == array {
      authors
    } else if type(authors) in (str, content, none) {
      (authors, )
    } else {
    ...
    short-authors : if type(short-authors) in (none,) {
      ...
      short-authors
    } else if type(short-authors) in (str,content){
      (short-authors, )
